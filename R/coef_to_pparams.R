...
coef_to_pparams = function(coef){
  coef_tibble = tibble::tibble(sp = names(coef), value = as.numeric(coef)) %>%
    tidyr::separate(sp, into = c("param", "unit"), sep = "\\[",
                    fill = "right") %>%
    dplyr::mutate(unit = gsub(pattern = "\\]", "", x = unit))
  shared_tibble = dplyr::filter(coef_tibble, is.na(unit))
  specific_tibble = dplyr::filter(coef_tibble, !is.na(unit))
